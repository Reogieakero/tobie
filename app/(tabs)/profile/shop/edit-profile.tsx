...
export default function EditShopProfile() {
  ...
  const { formData, setFormData, handleNameChange, handleUpdate, loading, fetching, isEditable, setIsEditable, checkCooldown } = useEditShop();
  const [showConfirm, setShowConfirm] = useState(false);

  const onEditPress = () => {
    const { canEdit, remainingDays } = checkCooldown();
    if (!canEdit) {
      showToast("Locked", `You can edit again in ${remainingDays} days.`, "info");
      return;
    }
    setShowConfirm(true);
  };
    ...
    <View style={styles.container}>
      <Stack.Screen 
        options={{
          ...
          headerLeft: () => (
            <TouchableOpacity onPress={() => router.replace('/(tabs)/profile/shop/settings')} style={{ marginLeft: 16 }}>
              <Ionicons name="arrow-back" size={24} color="#111" />
            </TouchableOpacity>
          ),
          headerRight: () => !isEditable && (
            <TouchableOpacity onPress={onEditPress} style={{ marginRight: 16 }}>
      ...
      <Modal visible={showConfirm} transparent animationType="fade">
        <View style={styles.overlay}>
          <View style={styles.card}>
            ...
            <Text style={styles.modalSub}>Once updated, you won't be able to change this for another 7 days.</Text>
            <View style={styles.actions}>
              ...
              <TouchableOpacity 
                ...
                onPress={() => { setIsEditable(true); setShowConfirm(false); }}
              >
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={[styles.form, !isEditable && { opacity: 0.5 }]}>
          ...
          <View style={styles.linkPreview}>
            ...
            <Text style={styles.linkText}>{formData.link}</Text>
          </View>
          ...
          <View style={styles.categoryGrid}>
            {CATEGORIES.map((cat) => (
              ...
              >
                <Text style={[styles.categoryLabel, formData.category === cat && styles.activeLabel]}>{cat}</Text>
              </TouchableOpacity>
            ))}
          </View>
